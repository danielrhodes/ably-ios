    ...
    override func spec() {
        describe("Channel") {
            ...
            it("should process all incoming messages and presence messages as soon as a Channel becomes attached") {
                ...
                expect(channel2.state).toEventually(equal(ARTChannelState.Attached), timeout: testTimeout)
            ...
            context("EventEmitter and states") {
                ...
                it("should implement the EventEmitter and emit events for state changes") {
                    ...
                    waitUntil(timeout: testTimeout) { done in
                        ...
                        channel.on(.Error) { stateChange in
                            ...
                            expect(reason.message).to(equal("channel has detached"))
                            expect(stateChange.current).to(equal(ARTChannelState.Detached))
                            done()
                        }
                        channel.attach()
                    }
                    ...
                    if states.count != 4 {
                        fail("Expecting 4 states; got \(states)")
                        return
                    }

                    expect(states[0].rawValue).to(equal(ARTChannelState.Attaching.rawValue), description: "Should be ATTACHING state")
                    expect(states[1].rawValue).to(equal(ARTChannelState.Attached.rawValue), description: "Should be ATTACHED state")
                    expect(states[2].rawValue).to(equal(ARTChannelState.Detaching.rawValue), description: "Should be DETACHING state")
                    expect(states[3].rawValue).to(equal(ARTChannelState.Detached.rawValue), description: "Should be DETACHED state")
                }
                ...
                it("should implement the EventEmitter and emit events for FAILED state changes") {
                    ...
                    if states.count != 3 {
                        fail("Expecting 3 states; got \(states)")
                        return
                    }

                    expect(states[0].rawValue).to(equal(ARTChannelState.Attaching.rawValue), description: "Should be ATTACHING state")
                    expect(states[1].rawValue).to(equal(ARTChannelState.Failed.rawValue), description: "Should be FAILED state")
                    // ERROR event
                    expect(states[2].rawValue).to(equal(ARTChannelState.Failed.rawValue), description: "Should be FAILED state")
                }
            ...
            describe("attach") {
                ...
                context("results in an error if the channel state is") {
                    ...
                    it("DETACHING") {
                        ...
                        expect(channel.state).toEventually(equal(ARTChannelState.Attached), timeout: testTimeout)
                        channel.detach()
                        expect(channel.state).to(equal(ARTChannelState.Detaching))
                ...
                it("should transition the channel state to FAILED if ATTACHED ProtocolMessage is not received") {
                    ...
                    let start = NSDate()
                    expect(channel.state).toEventually(equal(ARTChannelState.Failed), timeout: testTimeout)
                    expect(channel.errorReason).toNot(beNil())
            ...
            describe("detach") {
                ...
                it("if state is INITIALISED, DETACHED or DETACHING nothing is done") {
                    ...
                    channel.attach()
                    expect(channel.state).toEventually(equal(ARTChannelState.Attaching), timeout: testTimeout)
            ...
            describe("publish") {
                ...
                it("should encode messages in the same way as the RestChannel") {
                    ...
                    expect(realtimeChannel.state).toEventually(equal(ARTChannelState.Attached), timeout: testTimeout)
                    ...
                    for cryptoTest in [CryptoTest.aes128, CryptoTest.aes256] {
                        it("using \(cryptoTest) ") {
                            ...
                            waitUntil(timeout: testTimeout) { done in
                                ...
                                channel.on(.Error) { stateChange in
                                    guard let error = stateChange?.reason else {
                                        fail("Reason error is nil"); partlyDone(); return
                                    }
            ...
            context("history") {
                ...
                context("supports the param untilAttach") {
                    ...
                    it("should retrieve messages prior to the moment that the channel was attached") {
                        ...
                        let channel1 = client1.channels.get("test")
                        channel1.attach()
                        expect(channel1.state).toEventually(equal(ARTChannelState.Attached), timeout: testTimeout)
