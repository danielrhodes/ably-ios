...
- (void)transition:(ARTChannelState)state status:(ARTStatus *)status {
    ...
    if (status.errorInfo) {
        [self emit:ARTChannelEventError with:stateChange];
    }
}
...
- (void)setAttached:(ARTProtocolMessage *)message {
    ...
    if (self.state == ARTChannelStateAttached) {
        if (message.error != nil) {
            ...
            [self emit:ARTChannelEventError with:[[ARTChannelStateChange alloc] initWithCurrent:self.state previous:self.state reason:message.error]];
        }
        return;
    }
    ...
    for (ARTMessage *m in message.messages) {
        ...
        if (msg.data && dataEncoder) {
            ...
            if (error != nil) {
                ...
                [self emit:ARTChannelEventError with:[[ARTChannelStateChange alloc] initWithCurrent:self.state previous:self.state reason:errorInfo]];
            }
        }
